<template>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const selectedChurch = ref<string>('');
      ...
      function emitStartDate() {
        this.$emit('startDateSelected', startDate.value);
      }
  
      function emitEndDate() {
        this.$emit('endDateSelected', endDate.value);
      }
  
      function emitChurch() {
        this.$emit('churchSelected', selectedChurch.value);
      }
  ...
  .filter-container {
    ...
    width: calc(100% - 30px); /* subtrai 15px de cada lado */
    max-width: calc(100% - 30px);
    margin: 15px auto; /* mantém uma margem de 15px em cada lado */
    display: flex;
    align-items: center;
